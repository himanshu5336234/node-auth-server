...
class OrderBookSnapshot$Type extends MessageType<OrderBookSnapshot> {
    constructor() {
        super("OrderBookSnapshot", [
            ...
            { no: 4, name: "bids", kind: "message", repeat: 2 /*RepeatType.UNPACKED*/, T: () => PriceLevel },
            { no: 5, name: "asks", kind: "message", repeat: 2 /*RepeatType.UNPACKED*/, T: () => PriceLevel }
        ]);
    }
    ...
    internalBinaryWrite(message: OrderBookSnapshot, writer: IBinaryWriter, options: BinaryWriteOptions): IBinaryWriter {
        ...
        for (let i = 0; i < message.bids.length; i++)
            PriceLevel.internalBinaryWrite(message.bids[i], writer.tag(4, WireType.LengthDelimited).fork(), options).join();
        /* repeated PriceLevel asks = 5; */
        for (let i = 0; i < message.asks.length; i++)
            PriceLevel.internalBinaryWrite(message.asks[i], writer.tag(5, WireType.LengthDelimited).fork(), options).join();
        let u = options.writeUnknownFields;
        if (u !== false)
            (u == true ? UnknownFieldHandler.onWrite : u)(this.typeName, message, writer);
        return writer;
...
class OrderBookUpdate$Type extends MessageType<OrderBookUpdate> {
    constructor() {
        super("OrderBookUpdate", [
            { no: 1, name: "symbol", kind: "scalar", T: 9 /*ScalarType.STRING*/ },
            { no: 2, name: "sequence", kind: "scalar", T: 4 /*ScalarType.UINT64*/, L: 0 /*LongType.BIGINT*/ },
            { no: 3, name: "bids", kind: "message", repeat: 2 /*RepeatType.UNPACKED*/, T: () => PriceLevel },
            { no: 4, name: "asks", kind: "message", repeat: 2 /*RepeatType.UNPACKED*/, T: () => PriceLevel }
        ]);
    }
    ...
    internalBinaryWrite(message: OrderBookUpdate, writer: IBinaryWriter, options: BinaryWriteOptions): IBinaryWriter {
        ...
        for (let i = 0; i < message.asks.length; i++)
            PriceLevel.internalBinaryWrite(message.asks[i], writer.tag(4, WireType.LengthDelimited).fork(), options).join();
        let u = options.writeUnknownFields;
        if (u !== false)
            (u == true ? UnknownFieldHandler.onWrite : u)(this.typeName, message, writer);
        return writer;
